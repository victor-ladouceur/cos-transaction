...
public class ProxyInvocationHandler implements InvocationHandler, Serializable {
	...
	public Object invoke(Object pProxy, Method pMethod, Object []pArgs) throws Throwable {
		if (isTransactionMethod(pMethod))
			throw new RuntimeException("Illegal call: only transaction can manipulate a resource");
		...
		if (transactions != null && transactions.size() > 0) {
			...
			if (interceptedInvocations > 0)
				System.out.println(
						String.format(
								"Invocation %s.%s() was intercepted by %d transaction%s",
